...
import com.sensetime.sensecode.jetbrains.raccoon.clients.responses.SenseChatCheckTokenResponse
import com.sensetime.sensecode.jetbrains.raccoon.clients.responses.SenseChatRefreshTokenResponse
import com.sensetime.sensecode.jetbrains.raccoon.clients.responses.SenseNovaCodeResponse
...
class SenseChatOnlyLoginClient : CodeClient() {
    ...
    private suspend fun checkSenseChatTokenAndGetUserName(token: String?): String = okHttpClient.newCall(
        ...
        response.takeIf { it.isSuccessful }?.body?.let { responseBody ->
            RaccoonClientJson.decodeFromString(SenseChatCheckTokenResponse.serializer(), responseBody.string())
            ...
            refreshTokenCredentials?.letIfFilled { user, password ->
                ...
                okHttpClient.newCall(
                    ...
                    response.takeIf { it.isSuccessful }?.body?.let { body ->
                        RaccoonClientJson.decodeFromString(SenseChatRefreshTokenResponse.serializer(), body.string())
