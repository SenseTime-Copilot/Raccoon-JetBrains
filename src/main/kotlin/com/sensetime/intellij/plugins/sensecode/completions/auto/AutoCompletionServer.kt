...
import com.sensetime.intellij.plugins.sensecode.topics.SENSE_CODE_EDITOR_CHANGED_TOPIC
...
class AutoCompletionServer(
...
) : SenseCodeEditorChangedListener, EditorFactoryListener, CaretListener, FocusChangeListener, Disposable {
    ...
    private fun triggerCompletion() {
        project?.let {
            (FileEditorManager.getInstance(it).selectedTextEditor as? EditorEx)?.let { editor ->
                SenseCodeActionUtils.getAction(ManualTriggerInlineCompletionAction::class)?.let { action ->
    ...
    private fun tryTriggerCompletion(): Long {
        val isAutoCompleteMode = SenseCodeSettingsState.instance.isAutoCompleteMode
        val autoCompleteDelayMs = SenseCodeSettingsState.instance.autoCompleteDelayMs.toLong()
